    ...
    T: Send + Sync + 'static,
{
        ...
        S: Into<String>,
    {
        ...
        if result.is_err() {
            error!(
                "Failed to set EventRepeater {}'s Weak self-reference because it was already set. This should never happen. Shutting down ungracefully to prevent further undefined behavior.",
                arc.event.name
            );
            unreachable!(
    ...
    pub async fn attach(&self, event: &Event<T>, buffer: usize) -> Result<(), AttachError> {
        ...
        let arc = match weak.upgrade() {
            ...
            None => {
                error!(
                    "EventRepeater {}'s Weak self-reference could not be upgraded to Arc while attaching event {}. This should never happen. Shutting down ungracefully to prevent further undefined behavior.",
