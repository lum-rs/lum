...
use serenity::{
    ...
    framework::{StandardFramework, standard::Configuration},
...
impl Service for DiscordService {
    ...
    async fn start(&mut self, _service_manager: Arc<ServiceManager>) -> Result<(), BoxedError> {
        ...
        if self.cache.set(Arc::clone(&client.cache)).is_err() {
            error!(
                "Could not set cache OnceLock because it was already set. This should never happen."
            );
        ...
        match &client.voice_manager {
            Some(voice_manager) => {
                if self.voice_manager.set(Arc::clone(voice_manager)).is_err() {
                    error!(
                        "Could not set voice_manager OnceLock because it was already set. This should never happen."
                    );
                    return Err(
        ...
        if self.ws_url.set(Arc::clone(&client.ws_url)).is_err() {
            error!(
                "Could not set ws_url OnceLock because it was already set. This should never happen."
            );
...
impl client::EventHandler for EventHandler {
    async fn ready(&self, _ctx: Context, data_about_bot: Ready) {
        ...
        if self.client.set(data_about_bot).is_err() {
            error!(
                "Could not set client OnceLock because it was already set. This should never happen."
            );
